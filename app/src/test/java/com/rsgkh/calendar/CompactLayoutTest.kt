...
class CompactLayoutTest {
    ...
    @Test fun headersAndAddButtonStayFixedWhileTheirListsScroll() {
        ...
        val fab = compose.onNodeWithTag("add-event").getUnclippedBoundsInRoot()
        val content = compose.onNodeWithTag("events-content").getUnclippedBoundsInRoot()
        assertEquals(32f, (content.right - fab.right).value, .5f)
        assertEquals(32f, (content.bottom - fab.bottom).value, .5f)
        ...
        assertEquals(fab, compose.onNodeWithTag("add-event").getUnclippedBoundsInRoot())
        compose.onNodeWithTag("event-year").performClick()
        compose.onNodeWithTag("event-year-options").assertIsDisplayed()
