...
class CompactLayoutTest {
    ...
    @Test fun settingsFitKhmerAndEnglishWithConsistentTitles() {
        ...
        for (k in listOf(true, false)) {
            ...
            compose.onNode(hasText(L.text("ui.cambodia_time_utc_7.6b9f2d", k)) and hasAnyAncestor(isPopup())).performClick()
            compose.onNodeWithTag("today-time-zone").assertTextContains(L.text("ui.cambodia_time_utc_7.6b9f2d", k))
