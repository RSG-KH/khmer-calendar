...
| `data/RecurringEvents.kt` | Parses the bundled event catalog and translates its rule definitions into engine rules |
...
Gradle downloads `khmer-calendar-engine-jvm-0.2.0.jar` directly from the release. The version is pinned in `gradle/libs.versions.toml`; `settings.gradle.kts` limits the release repository to that module. No engine checkout or manual download is needed.

Before building, `:app:verifyCalendarEngine` checks the JAR against the release's SHA-256:

```text
d1a1a2968a6358043387fe9c1b8993b8d16595e96aad48f27e236e6555b4d509
```
...
The release is resolved as an artifact without Maven metadata. Version 0.2.0's only runtime dependency is Kotlin stdlib, which the app already supplies through AGP's built-in Kotlin support. Both the app and engine produce Java 11 bytecode; the Android build uses JDK 25 and Kotlin 2.4.20.
