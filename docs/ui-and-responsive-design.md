...
Settings shows the version on a separate line and groups clickable Android and PWA repository labels below it. The Sources dialog opens with an orange delivery note (new event years and corrections arrive through app updates), followed by the calculation-coverage summary, then three credited source paragraphs: official government holiday sources, the bundled event archive ([Khmer Lunar Calendar](https://khmer-lunar-calendar.com/)), and the shared engine. Each paragraph links its source name in the accent color — the government source opens its list of calendar URLs in a dialog, while the archive and engine names open their sites directly. A collapsible “Open-source licenses” header has a leading chevron and trailing divider, all using the selected accent color; the full row toggles the bundled Apache 2.0 license and upstream MIT notices below it. It starts collapsed and announces its expanded or collapsed state to accessibility services. Calculation details belong in the engine documentation linked from [shared engine integration](shared-engine.md).
