...
- **Show western zodiac** (`showWesternZodiac`, on by default) — turning it off removes the Western zodiac line from the selected-date card and Productivity widget, the Big 3 table from date details, hides the Glance widget's zodiac badge, and omits the zodiac glyph from event-detail backgrounds while keeping the animal-year artwork. In Date details, the Big 3 table presents the engine-calculated Sun, Moon, and Rising sign (Ascendant) using the active time zone (Phnom Penh for Cambodia time, central meridian for local time). When viewing Today, all three signs are calculated with live clock time; for other dates, Sun and Moon are calculated (midday) and Rising sign displays a compact 🕒 button to set a custom time via the time picker. Once a time is set, it recalculates Sun, Moon, Rising, and Chinese Ganzhi Hour pillar for that date and time. Month-grid cells never show zodiac signs. An inverted `hideWesternZodiac` choice saved by an earlier build carries over when the setting is next written.
...
- **Show Chinese Ganzhi (干支)** (`showGanzhi`, on by default) — shows the engine-supplied Year, Month, Day, and Hour sign/clash table in date details and the animal/clash badges in the Glance widget. In Date details, the Hour column is calculated with live clock time for Today and displayed with a compact 🕒 button for other dates (matching the Western Rising sign), allowing the user to pick a time and calculate the hour pillar and its clash. The Year sign and clash animals, as well as the Western Sun sign, are emphasized in bold with accent color. Year and Month are available for 1900–2100; outside that range the table marks them unavailable while preserving Day.
