...
The **Settings → Astrology & Zodiac** card (after Calendar) holds four switches, all saved on the device:

- **Show western zodiac** (`showWesternZodiac`, on by default) — turning it off removes the Western zodiac line from the selected-date card and Productivity widget, the Big 3 table from date details, hides the Glance widget's zodiac badge, and omits the zodiac glyph from event-detail backgrounds while keeping the animal-year artwork. In Date details, the Big 3 table presents the engine-calculated Sun, Moon, and Rising sign (Ascendant) using the active time zone (Phnom Penh for Cambodia time, central meridian for local time). Before a time is selected for another date, Sun and Moon use noon and Rising shows a compact 🕒 picker button. Month-grid cells never show zodiac signs. An inverted `hideWesternZodiac` choice saved by an earlier build carries over when the setting is next written.
- **Use emoji for western zodiac** (`useEmojiForWesternZodiac`, off by default) — appears only while Western zodiac is enabled. It switches the Big 3 table in date details between zodiac symbols (emoji) and English sign names. The choice is independent: turning Western zodiac off and back on never resets it.
- **Show Chinese Ganzhi (干支)** (`showGanzhi`, on by default) — shows the engine-supplied Year, Month, Day, and Hour sign/clash table in date details and the animal/clash badges in the Glance widget. Before a time is selected for another date, the Hour sign and clash cells show compact 🕒 picker buttons. The Year sign and clash animals, as well as the Western Sun sign, are emphasized in bold with accent color. Year and Month are available for 1900–2100; outside that range the table marks them unavailable while preserving Day.
- **Use emoji for Ganzhi animals** (`useEmojiForGanzhiAnimals`, off by default) — appears only while Chinese Ganzhi is enabled. It switches the date-details table between the twelve animal emoji and English or Khmer animal names; Glance's Ganzhi badges always use emoji when visible. The choice is independent: turning Chinese Ganzhi off and back on never resets it.

Date details takes a snapshot of the current time in the **Today follows** zone when Today's popup opens. The time stays fixed while the popup is open; reopening it takes a fresh snapshot. Other dates start without a selected time. The header has a 🕒 picker button from the start, then shows the selected time beside it. Users can pick, change, or clear the time from the header or an available Rising/Hour cell. The selected time updates only the enabled table: Western Sun, Moon and Rising, Chinese Hour sign and clash, or both. Clearing time restores noon for Western Sun and Moon and the picker buttons for Rising and Hour. The table clock buttons size themselves to fit the emoji with compact padding, and the shared symbol slot expands when Khmer text or larger font settings need more space.

When one astrology switch is off, Date details does not compose or calculate that table. When both are off, it does not capture a time, offer a header picker, or call either optional astrology calculator; the core Khmer date and event calculations still run. The divider above the optional block disappears when no Buddhist holy day or shaving day is shown and both astrology switches are off.
...
- **Dedicated Widget Font Size**: All five widgets follow the dedicated **Settings → Widgets → Font size** option (`widgetFontScale`, 100% by default, shown only while widgets are enabled), fully decoupled from the in-app font size. The picker offers the full **80%–200%** range without a font-size cap. Responsive layouts may hide optional details as space gets tight; Planner measures text to pack event chips across available rows.
- **Add Widgets**: A full-width **Add widgets** button at the bottom of the enabled Widgets settings card opens a swipeable chooser. Each page shows a preview, widget name and target size. The Add action requests the selected widget through Android's launcher pinning flow when the launcher supports it.
...
### Preview Thumbnails
- Static PNG preview thumbnails for Focus, Productivity, Month, Planner, and Glance are placed in `res/drawable/` (Light) and `res/drawable-night/` (Dark). Planner and Glance also declare XML preview layouts that display those supplied images on Android 12 and newer, while `previewImage` remains available as a fallback. The in-app chooser uses the preview images and adjusts their neutral colors for dark mode while retaining accent and event colors.
