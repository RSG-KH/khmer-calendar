...
- **Productivity Widget (`ProductivityWidget`)**: 4×2 date details card showing the big day number, short weekday, short month, Khmer lunar date, Buddhist Era year, optional Western zodiac sign, and a Holy Day or Shaving Day label when applicable. Today's/tomorrow's event lists sit beside it. The sign follows its app setting and retains its English name in Khmer mode. Detail rows hide on narrow widths (< 330dp).
...
- **Glance Widget (`GlanceWidget`)**: 4×1, horizontally resizable to 2×1 and vertically resizable up to two rows. A date tile shows weekday, day number and timezone emoji on separate lines; three adjacent detail rows show today's Gregorian, lunar and traditional year information. At 2×1 the third detail row shows traditional animal year and Sak; at wider sizes it shows holy-day and optional zodiac/Ganzhi badges. The app name and version fill that row when it is otherwise empty. Its refresh button uses the shared pressed state.
